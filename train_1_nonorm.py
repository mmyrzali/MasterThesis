...
save_tr_code(args['save_dir'], config, 'train_1_nonorm.py')
...
#print(longs,lats)
xi, yi = m(longs, lats)
inp = np.reshape(dataset[11], (10, 25))
print(inp)
plt.figure()
cs = m.pcolor(xi, yi, inp, shading='nearest', cmap='coolwarm')  # , vmin=0, vmax=1 , vmin=np.min(dataset), vmax=np.max(dataset)
...
plt.figure()
cs = m.pcolor(xi, yi, inp2, shading='nearest', cmap='coolwarm')  # vmin=0, vmax=1 , vmin=np.min(dataset), vmax=np.max(dataset)
